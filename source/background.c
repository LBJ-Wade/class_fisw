...
int background_ncdm_distribution(
  ...
  /***********************************************************************/
  /* FI-SW models (fisw), implemented by D. C. Hooper, L. Lopez-Honorez, */
  /* Q. Decant, J. Heisig, as discussed in 2111.09321                    */
  /***********************************************************************/

  else if (pba->fisw_case != none){
    /* Planck Mass in GeV, used for internal computations */
    double M_planck = 1.220e19;

    /* First we consider the case of FI from decay (and scattering) */
    /* PSD equation for decays as in 1706.09909 */

    if ((pba->fisw_case == fi) || ((pba->fisw_case == fisw) && (n_ncdm == 0)) ){
      /* Declare internal variables for the FI case, used to simplify equations */
      double mA_fi, mB_fi, gB_fi, Gam_fi, gstar_fi, M0_fi, deltad_fi, deltam_fi;
      double coupling, deno_coupling, mass_top = 172.0;
      double m_DM = pba->m_ncdm_in_eV[n_ncdm]*1.e-9;

      /* Add some shortcuts, just to make the notation less cumbersome */
      mA_fi = pba->fisw_mA_fi;
      mB_fi = pba->fisw_mB_fi;
      gB_fi = pba->fisw_gB_fi;
      Gam_fi = pba->fisw_Gam_fi;
      gstar_fi = pba->fisw_gstar_fi;

      /* Calculate auxiliary quantities */
      deltad_fi = (pow(mB_fi,2)-pow(mA_fi,2))/pow(mB_fi,2);
      deltam_fi = mB_fi-m_DM;
      M0_fi = M_planck / 1.66 / sqrt(gstar_fi); /* The Planck Mass is rescaled for H= T^2/M0 */

      *f0 = 0;

      /* First we add the decay contribution */
      if ((pba->fisw_fi_dec_scat == dec) || (pba->fisw_fi_dec_scat == decscat)){
        /* Calculate psd of FI through decays */
        *f0 += 1.0/pow(2.*_PI_,3)*2.0*gB_fi*Gam_fi*M0_fi/pow(mB_fi,2)/pow(deltad_fi,3)*sqrt(_PI_/q)*exp(-q);
      }

      /* Now we add the scattering contribution */
      if ((pba->fisw_fi_dec_scat == scat) || (pba->fisw_fi_dec_scat == decscat)){
        /* This is model dependent, and so it is just left here as a hook.
         * To calculate the scattering term for your model, you need to derive
         * the relevant PSD contribution. See 2111.09321 for inspiration.
         * Once you have the term calculated, you can remove the warning below,
         * and add your contribution here: */
        *f0 +=0;
        fprintf(stdout,"Warning: you requested scattering contribution to the FI PSD, but you have not yet added the corresponding equation. I will consider only the contribution to FI from decays.\n");
      }
    }

    /* Now we consider the case of SW from thermanl B decay */
    /* PSD equation for superWIMPs as in 2111.09321 */

    if ((pba->fisw_case == sw) || ((pba->fisw_case == fisw) && (n_ncdm == 1)) ){
      /* Declare internal variables for the SW case, used to simplify equations */
      double mA_sw, mB_sw, gB_sw, Gam_sw, gstar_sw, YFO_sw, M0_sw, deltad_sw, C_sw;

      /* Add some shortcuts, just to make the notation less cumbersome */
      mA_sw = pba->fisw_mA_sw;
      mB_sw = pba->fisw_mB_sw;
      gB_sw = pba->fisw_gB_sw;
      Gam_sw = pba->fisw_Gam_sw;
      gstar_sw = pba->fisw_gstar_sw;
      YFO_sw = pba->fisw_YFO_sw;

      /* Calculate auxiliary quantities */
      deltad_sw = (pow(mB_sw,2)-pow(mA_sw,2))/pow(mB_sw,2);
      M0_sw = M_planck / 1.66 / sqrt(gstar_sw); /* The Planck Mass is rescaled for H= T^2/M0 */
      C_sw = gstar_sw*YFO_sw*Gam_sw*M0_sw/pow(mB_sw,2)/deltad_sw*pow(2*_PI_,3.0/2.0)*2*pow(_PI_,2.0)/45.0;

      /* Calculate psd of SW */
      *f0 = 1.0/pow(2*_PI_,3)*4.0*sqrt(_PI_*Gam_sw*M0_sw/pow(mB_sw,2))*C_sw/(pow(deltad_sw,2.0)*q)*exp(-pow(q,2.0));
    }
  }
