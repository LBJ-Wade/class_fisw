...
int input_read_parameters_species(struct file_content * pfc,
  ...
  if (N_ncdm > 0){
    ...
    /** 5.c.1) (optional) p.s.d.-parameters */
    /* Read */
    parser_read_list_of_doubles(pfc,"ncdm_psd_parameters",&entries_read,&(pba->ncdm_psd_parameters),&flag1,errmsg);

    /** 5.c.2) FISW case as described in 2111.09321 */
    /* Read FISW flag and assign case */
    class_call(parser_read_string(pfc,"fisw_case",&string1,&flag1,errmsg),
               errmsg,
               errmsg);

    if (flag1 == _TRUE_) {
      if ((strstr(string1,"fi") != NULL) || (strstr(string1,"FI") != NULL)) {
        pba->fisw_case = fi;
      }
      else if ((strstr(string1,"sw") != NULL) || (strstr(string1,"SW") != NULL)) {
        pba->fisw_case = sw;
      }
      else if (strstr(string1,"both") != NULL) {
        pba->fisw_case = fisw;
      }
        ...
        class_stop(errmsg,
                   "Could not interpret case requested for FI/SW, please specify either fi, sw, or both.");
      }

      /* Check if we want decays, scatterings, or both for FI */
      if ((pba->fisw_case == fi) || (pba->fisw_case == fisw)){
        class_call(parser_read_string(pfc,"fi_dec_or_scat",&string2,&flag2,errmsg),
                   errmsg,
                   errmsg);

        if ((strstr(string2,"dec") != NULL) || (strstr(string2,"decay") != NULL)) {
          pba->fisw_fi_dec_scat = dec;
        }

        else if ((strstr(string2,"scat") != NULL) || (strstr(string2,"scattering") != NULL)) {
          pba->fisw_fi_dec_scat = scat;
        }

        else{
          pba->fisw_fi_dec_scat = decscat;
        }
      }
    }

    if (pba->fisw_case != none) {
      /* First we do the easier case of only FI or SW */
      if ((pba->fisw_case == fi) || (pba->fisw_case == sw) ){
        class_read_double_one_of_two("mA","mA_fi", pba->fisw_mA_fi);
        class_read_double_one_of_two("mB","mB_fi", pba->fisw_mB_fi);
        class_read_double_one_of_two("gB","gB_fi", pba->fisw_gB_fi);
        class_read_double_one_of_two("Gam","Gam_fi", pba->fisw_Gam_fi);
        class_read_double_one_of_two("gstar","gstar_fi", pba->fisw_gstar_fi);
        class_read_double_one_of_two("mA","mA_sw", pba->fisw_mA_sw);
        class_read_double_one_of_two("mB","mB_sw", pba->fisw_mB_sw);
        class_read_double_one_of_two("gB","gB_sw", pba->fisw_gB_sw);
        class_read_double_one_of_two("Gam","Gam_sw", pba->fisw_Gam_sw);
        class_read_double_one_of_two("gstar","gstar_sw", pba->fisw_gstar_sw);
        class_read_double_one_of_two("YFO","YFO_sw", pba->fisw_YFO_sw);
      }

      /* Now we do the case with both FI and SW */
      else {
        /* We check if the user wants the same values for FI and SW */
        class_call(parser_read_string(pfc,"fisw_same",&string1,&flag1,errmsg),
                   errmsg,
                   errmsg);

        /* Here we do the case where the user is passing separate values for both */
        if ( (flag1 == _TRUE_) && ((string_begins_with(string1,'n')) || (string_begins_with(string1,'N'))) ) {
          class_read_double("mA_fi", pba->fisw_mA_fi);
          class_read_double("mB_fi", pba->fisw_mB_fi);
          class_read_double("gB_fi", pba->fisw_gB_fi);
          class_read_double("Gam_fi", pba->fisw_Gam_fi);
          class_read_double("gstar_fi", pba->fisw_gstar_fi);
          class_read_double("mA_sw", pba->fisw_mA_sw);
          class_read_double("mB_sw", pba->fisw_mB_sw);
          class_read_double("gB_sw", pba->fisw_gB_sw);
          class_read_double("Gam_sw", pba->fisw_Gam_sw);
          class_read_double("gstar_sw", pba->fisw_gstar_sw);
          class_read_double_one_of_two("YFO","YFO_sw", pba->fisw_YFO_sw);

          /* TODO: add check that the user has passed both _fi and _sw */

        }

        /* Finally we deal with the case where FI and SW have the same parameters */
        /* Note that by default we are assuming that the two are the same */
        else {
          class_read_double("mA", pba->fisw_mA_fi);
          class_read_double("mB", pba->fisw_mB_fi);
          class_read_double("gB", pba->fisw_gB_fi);
          class_read_double("Gam", pba->fisw_Gam_fi);
          class_read_double("gstar", pba->fisw_gstar_fi);
          class_read_double_one_of_two("YFO","YFO_sw", pba->fisw_YFO_sw);
          /* Complete the list of parameters */
          pba->fisw_mA_sw = pba->fisw_mA_fi;
          pba->fisw_mB_sw = pba->fisw_mB_fi;
          pba->fisw_gB_sw = pba->fisw_gB_fi;
          pba->fisw_Gam_sw = pba->fisw_Gam_fi;
          pba->fisw_gstar_sw = pba->fisw_gstar_fi;
        }
      }
    }
